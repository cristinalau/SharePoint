...
url_normalized AS (
    SELECT /*+ MATERIALIZE */
           ...
           LOWER(REGEXP_SUBSTR(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(TRIM(url_pref), '[?#].*$', ''), '/+$', ''), '^https?://', ''), '^[^/]+')) AS domain_only
      FROM url_prefixes
),
url_match_terms AS (
    SELECT /*+ MATERIALIZE */ DISTINCT match_term
      FROM (
            /* Best path-level term: handles http/https, query params, fragments, and slash differences */
            ...
            /* Domain-level fallback: broader match on the same site/domain */
            SELECT domain_only AS match_term
              FROM url_normalized
             WHERE domain_only IS NOT NULL
               AND domain_only <> ''
           )
),
